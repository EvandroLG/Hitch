var Hitch = (function(global, doc){
	...
	var 
		...
		matchesSelector = (function(){
			...
			if(head.matchesSelector){ vendor = ""; matches = 'matchesSelector'; }
				...
				return function(e,sel){
					...
					return toArray(doc.querySelectorAll(sel)).indexOf(e) !== -1;
				};
			}
		...
		addMod = function(t){
			...
			while ((node1 = node1.parentNode)) {
				...
				while (i--) {
					...
					if(!iNode.tagName){ return doc.getElementsByTagName('body')[0]; }
					if(node1[method]){
		...
		throttleRetest = function(optionalT){
			...
			throttleId = setTimeout(function(){
				...
				if(modsQueue.length>1 || !(optionalT || {}).attrName){
					...
					if(el.tagName==='HTML'){ el = doc.getElementsByTagName('BODY')[0]; }
		...
		testSubtree = function(t){
			var targ = (t.target) ? t : { target: (t || doc.getElementsByTagName('body')[0]), type: 'DOMNodeInserted'};
		...
		normalizedUrl = function() {
			var location = global.location.href.split("?")[0].split("/");
			location.pop();
		...
		hitches = {
			...
			wire: function(){
				...
				applyStateRules = function(hitchName,segName,seg,origElement,toggleType,type){
					...
					stateRulesThrottle =  setTimeout(function(){
						...
						if(matchesSelector(origElement,selector)){
							potentialMatches = doc.querySelectorAll(segName);
				...
				doc[Hitch.events.getAttacherFnName()]('mouseover',function(e){delegateStateRules(e,"in","hover");});
				doc[Hitch.events.getAttacherFnName()]('mouseout',function(e){delegateStateRules(e,"out","hover");});
				doc[Hitch.events.getAttacherFnName()]('focus',function(e){delegateStateRules(e,"in","focus");},true);
				doc[Hitch.events.getAttacherFnName()]('blur',function(e){delegateStateRules(e,"out","focus");},true);

				if(doc.addEventListener){ // IE8 grafts attribute modification... what about tree mod?
		...
		scanForWidgets = function(root,complete){
			...
			try{
				resources = query(root || doc,'[data-hitch-widget]');
		...
		scanCSS = function(root, complete){
			...
			try{
				...
				resources = query((root||doc),'[data-hitch-interpret]:not([data-hitch-parsed])');
	...
	if((doc.head && doc.head.webkitMatchesSelector) || emulate){
